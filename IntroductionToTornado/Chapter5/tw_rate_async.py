...
define("port",default=8000,help="run otm the given port",type=int)
...
class IndexHandler(tornado.web.RequestHandler):
    ...
    def on_response(self,response):
        ...
        self.write(r'<div style="text-alig:center>'
                   r'<div style="font-size:72px">{}</div>'
                   r'<div style="font-size:144px">{}</div>'
                   r'<div style="fond-size:24px>tweets per sec</div>'
                   r'</div>'.format(self.get_argument("q"),tweets_per_second))
        self.finish()
